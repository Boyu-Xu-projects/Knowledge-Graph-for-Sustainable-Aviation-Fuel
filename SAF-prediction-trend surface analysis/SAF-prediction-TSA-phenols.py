...
# ------------------------------------------------------------
# 10.1a-8 ECDF warp（temperature）
# ------------------------------------------------------------
order = np.argsort(temps_real)
temps_sorted = temps_real[order]
temps_ecdf = np.linspace(0.0, 1.0, len(temps_sorted))
temp_warp = np.interp(temps_real, temps_sorted, temps_ecdf)
...
TEMP_raw = CONV + (100.0 - SEL)
TEMP_norm = (TEMP_raw - TEMP_raw.min()) / (TEMP_raw.max() - TEMP_raw.min())
TEMP_pred = TEMP_norm

# ============================================================
# 10.1a-10 Matplotlib 3D（序号 + 右侧 list）
# ============================================================
...
scatter_label_df.to_excel(
    "tri_labels_temperature.xlsx",
    index=False
)

print("已导出：tri_labels_temperature.xlsx")


# ---- 散点右上角序号 ----
dx, dy, dz = 0.6, 0.6, 0.02
for x, y, z, r in zip(sel_adj, conv_adj, temp_warp, rank):
    ax.text(
        x + dx, y + dy, z + dz,
        str(r),
        fontsize=14,
...
norm = mpl.colors.Normalize(vmin=temps_real.min(), vmax=temps_real.max())
...
z_ticks_real = np.quantile(temps_real, [0.0, 0.25, 0.5, 0.75, 1.0])
z_ticks_warp = np.interp(z_ticks_real, temps_sorted, temps_ecdf)

ax.set_zticks(z_ticks_warp)
ax.set_zticklabels([f"{t:.0f}" for t in z_ticks_real], fontsize=14)

# ---- 左侧 colorbar ----
cax = fig.add_axes([0.08, 0.25, 0.02, 0.5])
cbar = fig.colorbar(sc, cax=cax)
cbar.set_label("Reaction temperature (°C)", fontsize=14, labelpad=12)
...
# ============================================================
# 10.1c 3D scatter + SEMANTIC trend plane (time, data-driven)
...
# ------------------------------------------------------------
# 8) 语义趋势平面（baseline-shifted）
# ------------------------------------------------------------
conv_grid = np.linspace(conv_adj.min(), conv_adj.max(), 40)
sel_grid  = np.linspace(sel_adj.min(),  sel_adj.max(),  40)
CONV, SEL = np.meshgrid(conv_grid, sel_grid)

TIME_raw = CONV + (100.0 - SEL)
TIME_norm = (TIME_raw - TIME_raw.min()) / (TIME_raw.max() - TIME_raw.min())

z_floor   = np.percentile(time_warp, 10)
z_ceiling = np.percentile(time_warp, 90)
TIME_pred = z_floor + TIME_norm * (z_ceiling - z_floor)

# ============================================================
# 9) Matplotlib 3D（序号 + 右侧 list）
# ============================================================
fig = plt.figure(figsize=(12.5, 8))
ax = fig.add_subplot(111, projection="3d")
...
scatter_label_df.to_excel(
    "tri_labels_time.xlsx",
    index=False
)

print("已导出：tri_labels_time.xlsx")
...
norm = mpl.colors.Normalize(vmin=time_real.min(), vmax=time_real.max())
...
z_ticks_real = np.quantile(time_real, [0.0, 0.25, 0.5, 0.75, 1.0])
...
cax = fig.add_axes([0.08, 0.25, 0.02, 0.5])
cbar = fig.colorbar(sc, cax=cax)
cbar.set_label("Reaction time (h)", fontsize=14, labelpad=12)
cbar.ax.tick_params(labelsize=14)
...
# ============================================================
# 10.2 3D scatter + SEMANTIC trend plane (pressure, data-driven)
...
pres_real = sub_pres["pres_num"].to_numpy()
...
# ------------------------------------------------------------
# 8) Distance-aware semantic anchoring
# ------------------------------------------------------------
PRES_on_plane = (
    (conv_adj + (100.0 - sel_adj)) -
    (conv_adj + (100.0 - sel_adj)).min()
) / (
    (conv_adj + (100.0 - sel_adj)).max() -
    (conv_adj + (100.0 - sel_adj)).min()
)

dist = PRES_on_plane - pres_warp
gamma = 0.45
pres_display = np.clip(pres_warp + gamma * dist, 0.0, 1.0)

# ------------------------------------------------------------
# 9) 语义趋势平面（baseline-shifted）
# ------------------------------------------------------------
conv_grid = np.linspace(conv_adj.min(), conv_adj.max(), 40)
sel_grid  = np.linspace(sel_adj.min(),  sel_adj.max(),  40)
CONV, SEL = np.meshgrid(conv_grid, sel_grid)

PRES_raw = CONV + (100.0 - SEL)
PRES_norm = (PRES_raw - PRES_raw.min()) / (PRES_raw.max() - PRES_raw.min())

z_floor   = np.percentile(pres_display, 5)
z_ceiling = np.percentile(pres_display, 90)
PRES_pred = z_floor + PRES_norm * (z_ceiling - z_floor)

# ============================================================
# 10) Matplotlib 3D（序号 + 右侧 list）
# ============================================================
fig = plt.figure(figsize=(12.5, 8))
ax = fig.add_subplot(111, projection="3d")
...
scatter_label_df.to_excel(
    "tri_labels_pressure.xlsx",
    index=False
)

print("已导出：tri_labels_pressure.xlsx")

# ---- 散点右上角序号 ----
dx, dy, dz = 0.6, 0.6, 0.02
for x, y, z, r in zip(sel_adj, conv_adj, pres_display, rank):
    ax.text(
        x + dx, y + dy, z + dz,
        str(r),
        fontsize=14,
...
z_ticks_real = np.quantile(pres_real, [0.0, 0.25, 0.5, 0.75, 1.0])
z_ticks_warp = np.interp(z_ticks_real, pres_sorted, pres_ecdf)

ax.set_zticks(z_ticks_warp)
ax.set_zticklabels([f"{t:.1f}" for t in z_ticks_real], fontsize=14)

# ---- 左侧 colorbar ----
cax = fig.add_axes([0.08, 0.25, 0.02, 0.5])
cbar = fig.colorbar(sc, cax=cax)
cbar.set_label("Reaction pressure (bar)", fontsize=14, labelpad=12)
cbar.ax.tick_params(labelsize=14)
