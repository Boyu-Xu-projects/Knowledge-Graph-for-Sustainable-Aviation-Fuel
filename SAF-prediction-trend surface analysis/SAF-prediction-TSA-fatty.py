...
# ------------------------------------------------------------
# 10.1a-5 Temperature bins（data-driven）
# ------------------------------------------------------------
...
# ============================================================
# 10.1a-8 ECDF warp（temperature 轴）
# ============================================================
order_temp = np.argsort(temps_real)
temps_sorted = temps_real[order_temp]
...
# ============================================================
# 10.1a-10 Matplotlib 3D（序号 + 彩色 list）
# ============================================================
...
ax.set_zticklabels(
    [f"{t:.0f}" for t in z_ticks_real],
    fontsize=14
)

# ax.set_title(
#     f"Hydrogenation — {target_cat}\n"
#     "Selectivity vs. conversion vs. temperature (model predictions)"
# )
...
# ============================================================
# 10.1c 3D scatter + SEMANTIC trend plane (time, ECDF-scaled)
...
# ax.set_zticks(z_ticks_warp)
# ax.set_zticklabels([str(int(t)) for t in z_ticks_real], fontsize=14)
...
ax.set_zticklabels([str(int(t)) for t in z_ticks_real[keep]])
...
# ============================================================
# 10.2 3D scatter + LINEAR trend plane
...
pres  = sub_pres["pres_num"].to_numpy()
time  = sub_pres["time_num_filled"].to_numpy()

num_feats = np.column_stack([temps, pres, time])
num_scaled = scaler.transform(num_feats)
X_pred = np.hstack([num_scaled, context_emb])
...
# ------------------------------------------------------------
# 10.2-7 线性趋势平面（Pressure = f(conv, 100-sel)）
# ------------------------------------------------------------
...
PRES_pred = reg.predict(X_grid).reshape(CONV.shape)
...
for x, y, z, r in zip(sel_adj, conv_adj, pres, rank):
    ax.text(
        x + dx,
        y + dy,
        z + dz,
...
# ============================================================
# ★ 3) 右侧 list：序号黑色 + 名称颜色 = pressure
# ============================================================
cmap = plt.get_cmap("cividis")
norm = mpl.colors.Normalize(vmin=pres.min(), vmax=pres.max())
...
ax.set_xlabel("Predicted selectivity (%)", fontsize=14)
ax.set_ylabel("Predicted conversion (%)", fontsize=14)
ax.set_zlabel("Reaction pressure (bar)", fontsize=14, labelpad=10)

ax.view_init(elev=20, azim=60)
ax.tick_params(axis="both", labelsize=14)
ax.tick_params(axis="z", labelsize=14)

z_ticks_real = np.round(np.linspace(pres.min(), pres.max(), 5))
